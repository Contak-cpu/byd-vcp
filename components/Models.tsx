...
const ModelCard: React.FC<ModelCardProps> = ({ model, onMoreInfoClick }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 group">
      ...
      <div className="p-6">
        <h3 className="font-heading text-2xl font-bold text-gray-900 dark:text-white">{model.name}</h3>
        <p className="text-gray-600 dark:text-gray-400 mt-1">{model.category}</p>
        <div className="mt-4 flex justify-between items-center">
          <span className="font-bold text-lg text-primary-600">{model.price}</span>
          <p className="text-sm text-gray-600 dark:text-gray-400">Precio referencial</p>
        </div>
        <div className="mt-6 flex space-x-2">
           <button onClick={() => onMoreInfoClick(model)} className="flex-1 bg-gray-200 dark:bg-gray-600 text-gray-900 dark:text-white font-bold py-3 px-4 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors duration-300">
             Configurar
           </button>
           <button onClick={handleScrollToForm} className="flex-1 bg-primary-600 text-white font-bold py-3 px-4 rounded-md hover:bg-primary-700 transition-colors duration-300">
             Reservar
           </button>
        </div>
      </div>
